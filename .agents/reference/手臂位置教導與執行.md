...
```python
# 核心亮點：弧度總合保護 (避免撞擊桌面) 與 QoS 最佳化
def check_safety(self, q1, q2):
    # 利用「兩軸弧度總和」直接檢查是否會撞地，避免高度計算誤差
    # 根據實驗：174.5° + 60.2° = 234.7° 為臨界點，設定 234.0° 為上限
    current_sum = q1 + q2
    if current_sum > self.JOINT_SUM_MAX: # JOINT_SUM_MAX = 4.084 (234.0°)
        self.node.get_logger().warn(
            f"🛡️ 觸發弧度保護：Q1+Q2={math.degrees(current_sum):.1f}°"
        )
        return self.target_positions[0], self.target_positions[1]
    return q1, q2

# 訂閱者使用感測器專用 QoS，提高連線穩定度
