...
### 1.2 自由度與控制維度
- **位形空間 (Configuration Space)**: 3 個控制維度 (x, y, θ)
- **速度空間 (Velocity Space)**: 2 個自由度（線速度 v、角速度 ω）
...
- `arm_1_joint`: 30° ~ 210°（需確保避免與地板碰撞）
- `arm_2_joint`: 0° ~ 240°（需確保避免與地板碰撞）
- `gripper_joint` (夾爪):
  - **全開角度**: 240°
  - **閉合極限**: 168°
  - ⚠️ **警告**: 嚴禁指令角度小於 168°，避免過夾導致馬達毀損！

### 3.2 保護機制與自動釋放
- **防過壓機制**: 完成抓取後，必須於 **0.5 秒** 後自動退回 **2°**，以釋放堵轉壓力。

### 3.3 溫度防護機制 (Thermal Protection)
監控話題 `/arm_joint_temperatures`（大臂、小臂、夾爪）：
- **過熱警告**: ≥ 65°C 發出警告訊息。
- **緊急停機 (E-Stop)**: ≥ 70°C 必須立即停止所有發送指令，攔截動作。
- **自動恢復**: 溫度回落至 60°C 以下方可解除鎖定。
