...
# 2. 直行與定位校正 (GO) 任務參數
WALL_TO_BRIDGE_CENTER = 1.18       # 起點後牆到橋中心總距離，預設 200cm (2.0)
...
class BridgeNavigation(Node):
    ...
    def run_cross_bridge(self, target_val=CROSS_BRIDGE_DISTANCE, speed=MOVE_ODOM_SPEED, direction=1):
        self.current_mode = 'idle'
        rate = self.create_rate(20)
        start_x = self.odom_x
        start_y = self.odom_y
        ...
        dir_str = "前進" if direction >= 0 else "倒退"
        print(f"\n\033[1;36m🚀 [MOVE_ODOM] 里程{dir_str}過橋：目標最大距離 {target_val*100:.1f} cm，速度 {actual_speed:.3f} m/s...\033[0m")
        ...
        while rclpy.ok():
            ...
            if self.current_pitch is not None:
                if direction >= 0:
                    ...
                    if not downslope_triggered and pitch_deg >= DOWNSLOPE_PITCH_THRESHOLD:
                        ...
                        print(f"\n\033[1;33m🚨 [仰角監測] 偵測到小車開始下坡 (Pitch: {pitch_deg:+.1f}°)，等待回正...\033[0m")
                    
                    if downslope_triggered and pitch_deg <= LEVEL_PITCH_THRESHOLD:
                        is_aligned_level = True
                else:
                    # 倒退時：上橋 (車頭朝下) 仰角為正，下坡 (車頭朝上) 仰角為負
                    if not downslope_triggered and pitch_deg <= -DOWNSLOPE_PITCH_THRESHOLD:
                        downslope_triggered = True
                        print(f"\n\033[1;33m🚨 [仰角監測] 偵測到小車開始下坡 (車頭朝上，Pitch: {pitch_deg:+.1f}°)，等待回正...\033[0m")
            ...
            if is_aligned_level:
                print(f"\n\033[1;32m🎉 偵測到仰角降回水平 (Pitch: {pitch_deg:+.1f}°)，已成功下橋！繼續保持當前速度前進 1 秒...\033[0m")
                # 保持原速度發送指令維持 1 秒 (迴圈 20 次，每次 50ms)
                msg = TwistStamped()
                msg.header.stamp = self.get_clock().now().to_msg()
                msg.header.frame_id = 'base_link'
                ...
                for _ in range(20):
                    self.cmd_pub.publish(msg)
                    time.sleep(0.05)
                print(f"\033[1;32m🎉 額外前進 1 秒完成，即將煞停小車！\033[0m")
                break
    ...
    def main_task_flow(self, side='right', first_go_dist=1.28):
        ...
        if not self.run_go_to_center(target_val=first_go_dist, side=side): return
        time.sleep(0.5)

        # 2. 運動測試：原地左轉 (純里程計，可自訂角度) ，角度93
        if not self.run_turn_left_odom(target_deg=92.0): return
        time.sleep(0.5)

        # 3. 純里程計移動過橋 (自訂距離/下坡仰角回正監測/小車車頭向下) (MOVE_ODOM) ，距離270cm
        if not self.run_cross_bridge(target_val=2.70, speed=0.2, direction=1): return
        time.sleep(0.5)

        # 4. 純里程計移動小車倒退過橋 (自訂距離/下坡仰角回正監測/這時小車會變成車頭向上) (MOVE_ODOM) ，距離270cm
        if not self.run_cross_bridge(target_val=2.70, speed=0.2, direction=-1): return
        time.sleep(0.5)

        # 5. 純里程計小車後退 (自訂距離/速度/打滑補償) (MOVE_DISTANCE) ，距離67cm
        if not self.run_move_distance(target_val=0.40, speed=0.2, direction=-1): return
        time.sleep(0.5)

        # 6. 運動測試：原地右轉 (純里程計，可自訂角度)，88度
        if not self.run_turn_right_odom(target_deg=95.0): return
        time.sleep(0.5)

        # 7. 純里程計小車後退 (自訂距離/速度/打滑補償) (MOVE_DISTANCE) ，距離125cm
        if not self.run_move_distance(target_val=1.00, speed=0.2, direction=-1): return
        time.sleep(0.5)
            
        print(f"\n\033[1;32m🎉🎉🎉 恭喜！過橋及倒車返回原點全自動化導航任務圓滿完成！\033[0m")
...
def main():
    ...
    # 讓使用者輸入第一次直走距離
    try:
        user_input = input("請輸入第一次直走的距離 (公尺, 預設 1.28): ")
        first_go_dist = float(user_input) if user_input.strip() else 1.28
    except ValueError:
        print("輸入無效，使用預設值 1.28")
        first_go_dist = 1.28
    ...
    try:
        ...
        node.main_task_flow(side=side, first_go_dist=first_go_dist)
