...
# 2. 直行與定位校正 (GO) 任務參數
WALL_TO_BRIDGE_CENTER = 2.0       # 起點後牆到橋中心總距離，預設 200cm (2.0)
STAGE1_GO_DISTANCE = 0.10         # GO 階段 1 里程計前進距離 (避開雷達死區)，預設 10cm (0.10)
GO_SPEED = 0.10                   # 直行速度 (m/s)，預設 0.10 m/s
...
MOVE_ODOM_SPEED = 0.12            # 過橋時的前進速度，預設 0.12 m/s
...
class BridgeNavigation(Node):
    ...
    def main_task_flow(self, side='right'):
        ...
        if self.odom_x is None:
            print(f"\n{COLOR_RED}❌ 里程計感測器同步逾時，請確認底盤驅動是否已啟動！{COLOR_RESET}")
            return
            
        print(f"\n{COLOR_GREEN}✅ 感測器同步完成，開始載入起點配置...{COLOR_RESET}")
        
        side_display = "左側起點 (左側有牆)" if side == 'left' else "右側起點 (右側有牆)"
        print(f"👉 {COLOR_BOLD}{COLOR_GREEN}起始設定：小車位於【{side_display}】{COLOR_RESET}")
        print(f"{COLOR_BOLD}⚠️  提示：在運動過程中，隨時按下【空白鍵】即可立即停止小車並終止任務！{COLOR_RESET}\n")
        time.sleep(1.0)
        
        # 1. 執行 GO (直行到橋中心 - 輸入距離 WALL_TO_BRIDGE_CENTER，帶入判定邊 side)
        if not self.run_go_to_center(target_val=WALL_TO_BRIDGE_CENTER, side=side):
            print(f"\n{COLOR_RED}❌ 步驟 1/3 (GO) 執行失敗或被中斷。任務終止。{COLOR_RESET}")
            return
            
        time.sleep(1.0)
        
        # 2. 執行轉彎 (左側起點 L 則右轉上橋，右側起點 R 則左轉上橋，並做對應校正)
        if side == 'left':
            success = self.run_turn_right_90()
            step_name = "TURN_RIGHT"
        else:
            success = self.run_turn_left_90()
            step_name = "TURN_LEFT"
            
        if not success:
            print(f"\n{COLOR_RED}❌ 步驟 2/3 ({step_name}) 執行失敗或被中斷。任務終止。{COLOR_RESET}")
            return
            
        time.sleep(1.0)
        
        # 3. 執行 MOVE_ODOM (前進過橋並仰角回正煞停 - 輸入距離 CROSS_BRIDGE_DISTANCE)
        if not self.run_cross_bridge(target_val=CROSS_BRIDGE_DISTANCE, speed=MOVE_ODOM_SPEED):
            print(f"\n{COLOR_RED}❌ 步驟 3/3 (MOVE_ODOM) 執行失敗或被中斷。任務終止。{COLOR_RESET}")
            return
            
        print(f"\n{COLOR_GREEN}🎉🎉🎉 恭喜！過橋自動導航任務圓滿完成！{COLOR_RESET}")
