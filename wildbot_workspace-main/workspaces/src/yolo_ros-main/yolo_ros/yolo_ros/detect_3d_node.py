...
class Detect3DNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self._marker_pub = self.create_publisher(Marker, "/yolo/target_marker", 10)
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.depth_sub = message_filters.Subscriber(
            self, Image, "/camera/depth/raw", qos_profile=self.depth_image_qos_profile
        )
    ...
    def on_detections(
        ...
        # 取得距離最近的黑熊 (相機光學座標系：Z是深度/往前，X是左右)
        target_detections = [d for d in new_detections_msg.detections if d.class_name == 'black_bear']
        if target_detections:
            # 距離平方 = X^2 + Z^2
            det = min(target_detections, key=lambda d: d.bbox3d.center.position.x**2 + d.bbox3d.center.position.z**2)
            marker = Marker()
            marker.header = new_detections_msg.header
            ...
            # 直接使用修正後的 bbox3d 座標 (process_detections 已經處理好 base_link 和 map 的轉換)
            marker.pose.position.x = det.bbox3d.center.position.x
            marker.pose.position.y = det.bbox3d.center.position.y
            
            # Z 是高度。要抓黑熊底部(腳底)，把高度往下減 (size.z/2)
            marker.pose.position.z = det.bbox3d.center.position.z - (det.bbox3d.size.z / 2.0)
            
            # 安全機制：如果算出來低於 0，就當作貼在地上
    ...
    def convert_bb_to_3d(
        ...
        if detection.mask.data:
            ...
            # Create mask based on RGB resolution (assuming mask coordinates are in RGB scale)
            # Wait, we need to know the original mask scale, but we can just use the depth image shape directly
            # by scaling the mask coordinates.
            depth_h, depth_w = depth_image.shape[:2]
            
            # Since mask coordinates might be from a 640x480 image, we calculate scale
            # We assume bbox coordinates are on the same scale as mask
            # But the safest way is to just draw the mask on a depth-sized canvas by scaling the coordinates!
            # Let's find the max X and Y in the mask to guess its original resolution
            max_x = max(ele.x for ele in detection.mask.data) if detection.mask.data else depth_w
            scale_x = depth_w / 640.0 if max_x > depth_w else 1.0
            scale_y = depth_h / 480.0 if max_x > depth_w else 1.0

            scaled_mask_array = np.array(
                [[int(ele.x * scale_x), int(ele.y * scale_y)] for ele in detection.mask.data]
            )

            mask = np.zeros((depth_h, depth_w), dtype=np.uint8)
            cv2.fillPoly(mask, [np.array(scaled_mask_array, dtype=np.int32)], 255)

            # Get pixel coordinates for spatial weighting
            y_coords, x_coords = np.where(mask > 0)
            pixel_coords = np.column_stack([x_coords, y_coords])
            valid_depths = depth_image[mask > 0]
        ...
        # Convert to meters
        raw_depths = valid_depths.astype(np.float64)
        
        # 動態判斷除數：如果是 float32，通常已經是公尺 (Meters)
        actual_divisor = 1.0 if depth_image.dtype == np.float32 else self.depth_image_units_divisor
